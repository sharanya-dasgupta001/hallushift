...
def process_with_threads(dataset, process_func, max_workers):
    """
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        return list(tqdm(executor.map(process_func, dataset), total=len(dataset), desc="Processing dataset"))
...
def main():
    ...
    - Evaluate generated responses using BLEURT and other metrics.
    - Train a classifier if required.
    """
    ...
    parser.add_argument('--model_dir', type=str, default=None, help='Local directory containing model data.')
    ...
    MODEL = args.model_dir or MODELS_NAMES[args.model_name]
    ...
    if args.num_workers == 1:
        result = []
        for index, row in tqdm(enumerate(dataset), total=len(dataset), desc="Generating responses..."):
            if args.dataset_name in ['truthfulqa', 'triviaqa']:
                prompt = tokenizer(base_prompt.format(question=row['question']), return_tensors='pt').to("cuda") 
            elif args.dataset_name == 'tydiqa':
                prompt = tokenizer(base_prompt.format(context=row['context'], question=row['question']), return_tensors='pt').to("cuda") 
        ...
        result = process_with_threads(dataset, process_row, max_workers=args.num_workers)
    ...
    print("BLEURT model downloaded")

    print("Running BLEURT scoring...")
    os.system(
    ...
    if os.path.exists("llm_answer") and os.path.exists("answers") and os.path.exists("scores") and os.path.exists("id") :
        ...
        print("BLEURT Score files not found")
        exit()
